...
async function runOne(target: Target): Promise<{ ok: true; run: BackupRun } | { ok: false; error: string; run: BackupRun }> {
  const id = newId('bk');
  ...
  try {
    const dump = await runMongodump(target);
...
export async function runTargetById(targetId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  await applyRetention([target.dbName]);
...
export async function runAllTargets(): Promise<{ total: number; success: number; failed: number }> {
  ...
  if (dbNames.length) await applyRetention(dbNames);
  if (failures.length) await sendFailureReport(failures);
